...
- **Never edit generated files:** `packages/backend/src/generated/prisma/`, `packages/web/src/lib/api/fetch-generated/` and `packages/web/src/lib/api/schemas/`.
...
- **React contexts live in `packages/web/src/contexts/`** (`createContext` + provider + the trivial context accessor in one file, e.g. `coach-context.tsx`) — never in `components/`. **Custom hooks with logic live in `packages/web/src/hooks/`**, one file per hook (`use-<name>.ts`, e.g. `use-coach-chat.ts`); hooks that do more than expose a context belong there.
